...
public struct TextFieldRenderer: ViewRenderer {
  public static let type: ViewType = .textField
...
/// Internal view that maintains TextField state
...
#Preview {
  DebugView {
    VStackNode(spacing: 20, alignment: .leading) {
      TextNode("Basic TextField")
        .font(.headline)

      TextFieldNode("Enter your name")
        .onTextChange("textChanged")
        .onBlur("leftFieldBlurred")
        .padding(10)
        .background(ColorNode("#F0F0F0"))
        .cornerRadius(8)

      TextNode("TextField with initial value")
        .font(.headline)
      ...
      TextNode("TextField with message handlers")
        .font(.headline)

      TextFieldNode("Search...")
        .onTextChange(.dict([.action: "search"]))
        .onSubmit(.dict([.action: "submitSearch"]))
        .onBlur(.dict([.action: "searchBlurred"]))
        .padding(10)
        .background(ColorNode("#F0F0F0"))
        .cornerRadius(8)
    }
